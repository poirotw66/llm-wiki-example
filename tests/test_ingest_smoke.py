...
def test_ingest_handoff_smoke(tmp_path: Path) -> None:
    ...
    paths = cleanup.PATHS
    for directory in (paths.raw_inbox, paths.raw_originals, paths.raw_sources, paths.wiki_sources):
        directory.mkdir(parents=True, exist_ok=True)
    source = paths.raw_inbox / "brief.md"
    source.write_text("# Brief\n", encoding="utf-8")
    digest = cache.sha256_file(source)
    assert cache.cmd_lookup(SimpleNamespace(path=str(source), cache=str(paths.ingest_cache), force=False, require_miss=False)) == 0
    original = paths.raw_originals / "brief.md"
    original.write_bytes(source.read_bytes())
    archive = paths.raw_sources / "brief.md"
    ...
    assert review.cmd_append(SimpleNamespace(queue=str(paths.review_queue), title="confirm", reason="smoke", source="ingest", action="human_verify", related=[], id="smoke")) == 0
    assert cache.cmd_record(SimpleNamespace(path=None, cache=str(paths.ingest_cache), archive_slug="brief", source_page="wiki/sources/brief.md", sha256=digest, original_name=source.name, analysis_receipt=receipt, analysis_version="1", analysis_source_sha256=source_receipt, analysis_generated_by="agent/test", analysis_generated_at="2026-08-07T00:00:00Z")) == 0
