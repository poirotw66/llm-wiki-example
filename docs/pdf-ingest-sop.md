...
**預設路徑（fast）**：`pdftotext` 產出逐頁文字初稿 + 頁級 triage → **僅**視覺閘頁 `pdftoppm` + vision subagent。helper：`uv run python scripts/docling-pdf.py`（預設 `--engine fast`，**不**載入 Docling）。

**full／Docling**：**僅當使用者明確指定**（「用 full」「用 Docling」「`--engine docling`」）才啟用；Agent **不得**自行升級。模型見下方 **可選：Docling 安裝**。

### 本檔與 visual-source-conversion 分工

| 主題 | pdf-ingest-sop（本檔） | visual-source-conversion |
|------|------------------------|---------------------------|
| fast／docling、頁級 triage、`pdftoppm`、DPI、slug | **單一來源** | — |
...
本機 OCR（含 Docling／RapidOCR）對**繁體中文簡報**常出現簡體、缺字、亂碼。本倉政策：

- 文字層（`pdftotext`）夠用 → 以文字層初稿為主。
- 文字層空或極短 → **直接進視覺閘**，正文＋Visual Evidence 以 **subagent 讀圖**為準。
- **不要**為了救 OCR 另裝模型、調參或把 OCR 亂稿當 canonical；衝突時以圖為準。

**前置（安裝）— 預設 fast 只需 Poppler**

需已安裝 [uv](https://docs.astral.sh/uv/)（跑 helper／lint）。系統工具：`pdfinfo`／`pdftotext`／`pdftoppm`（poppler）；大圖偵測建議有 `pdfimages`。
...
# 確認 helper（預設 --engine fast，不需 Docling 模型）
uv run python scripts/docling-pdf.py --help
```

**可選：Docling 安裝**（僅在要用 `--engine docling` 時）
...
# 2) Docling 模型組 → models/docling/（已 gitignore；約 1.2GB）
...
| **下載來源** | RapidOCR 權重來自 **ModelScope**；layout／tableformer 等來自 **Hugging Face** |
| **CLI** | 請用 `uv run docling-tools ...`；直接打 `docling-tools` 常因 PATH 找不到而失敗 |

| 平台 | torch | 說明 |
|------|-------|------|
| **Intel macOS (x86_64)** | **`torch==2.2.2`**（PyTorch 最後支援版）+ `numpy<2` + `transformers<5` | CPU 可跑但偏慢；預設仍用 **fast**，除非使用者指定 Docling |
| **Apple Silicon／Linux** | 可用 `torch>=2.4` | 有 GPU／MPS 時 Docling 較快，仍須使用者指定才啟用 |

`--engine docling` 以 `artifacts_path=models/docling` 呼叫 Docling；缺模型時會提示下載指令。Docling 失敗時會 **自動後備** 為逐頁 `pdftotext`（stdout `engine: pdftotext-fallback`）；仍須對視覺閘頁跑 vision。
...
```bash
# 初稿 + 分流 JSON（stdout）；draft 預設寫入 <OS-temp>/<base-slug>-pdf-draft.md
...
# 僅使用者指定 full／Docling 時
uv run python scripts/docling-pdf.py "<path>.pdf" --engine docling --base-slug "<base-slug>"
```
...
# 可選 docling：優先裁切內嵌圖；抽不到再整頁 pdftoppm
uv run python scripts/docling-pdf.py "<path>.pdf" --engine docling \
  --base-slug "<base-slug>" --export-vision-assets --triage-only

# 強制整頁渲染（docling 引擎時亦可用）
uv run python scripts/docling-pdf.py "<path>.pdf" --export-vision-assets \
  --force-page-render --triage-only
...
| method | 意義 |
|--------|------|
| `pdftoppm_page` | 預設 fast、或 `--force-page-render` → 整頁 PNG |
| `docling_picture` | `--engine docling` 時內嵌圖或自頁面裁切的 figure |
...
合併 **文字初稿**（`pdftotext`；或可選 Docling）與 **vision 補寫**（資訊圖頁），新增 `raw/sources/<archive-slug>.md`（詳盡還原，非 wiki 摘要）。文首建議含：
...
- 原件 SHA-256：`…`
- 轉檔：pdftotext（fast；或 `--engine docling`）
...
| 要用 Docling 卻未安裝 | `uv sync --group pdf` + 模型下載（見 **可選：Docling 安裝**） |
| 只下 RapidOCR、缺 layout／tableformer | 下完整預設組：`uv run docling-tools models download -o models/docling`（勿只傳 `rapidocr`） |
| RapidOCR／Docling 模型找不到／又下載到 `~/.cache` | 固定目錄同上；helper 讀 `models/docling/`（`DOCLING_ARTIFACTS_PATH` 可覆寫） |
...
- [ingest-pipeline.md](./ingest-pipeline.md) — Ingest 管線對照（完整步驟見 PROMPTS／AGENTS）
- [okf.md](./okf.md) — `archive_slug` 與 resource 語意
- [Docling](https://github.com/docling-project/docling) — 可選本機文件解析（`--engine docling`）
